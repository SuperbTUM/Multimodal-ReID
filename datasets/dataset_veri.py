...
class VeRi(BaseImageDataset):
    ...
    def __init__(self, root='', verbose=True, **kwargs):
        ...
        path_car_type_train = osp.join(self.dataset_dir, 'train_label.xml')
        with open(path_car_type_train, 'rb') as xml:
            lines = xml.read()
        self.image_car_types_train = {}
        bs_data = BeautifulSoup(lines, 'xml')
        items = bs_data.find_all("Item")
        for item in items:
            vehicle_id = item.get("imageName")
            type_id = item.get("typeID")
            self.image_car_types_train[vehicle_id] = type_id

        path_car_type_test = osp.join(self.dataset_dir, 'test_label.xml')
        with open(path_car_type_test, 'rb') as xml:
            lines = xml.read()
        self.image_car_types_test = {}
        bs_data = BeautifulSoup(lines, 'xml')
        items = bs_data.find_all("Item")
        for item in items:
            vehicle_id = item.get("imageName")
